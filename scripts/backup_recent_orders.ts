...
const TARGET_DATES = ['2026-01-23', '2026-01-24']; // YYYY-MM-DD
...
async function backup() {
    ...
    sheet.eachRow((row, rowNumber) => {
        ...
        const startVal = row.getCell(5).value;
        let startDateStr = '';

        if (startVal instanceof Date) {
            startDateStr = startVal.toISOString().split('T')[0];
        } else if (typeof startVal === 'string') {
            startDateStr = startVal.split('T')[0];
        }

        // Check if date matches
        if (TARGET_DATES.includes(startDateStr)) {
    ...
    console.log(`Backup complete. Saved ${backedUpOrders.length} orders to ${BACKUP_FILE}`);
}
